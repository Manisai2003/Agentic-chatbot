...
/**
 * @langchain/mongodb internally does `this.collection.db.client.appendMetadata(...)`
 * inside the MongoDBAtlasVectorSearch constructor. That shape (`collection.db.client`)
 * only exists on `mongodb` driver v6.20.0+. If someone's node_modules has an older
 * `mongodb` package (e.g. hoisted by another dependency), collection.db is undefined
 * and the constructor throws a confusing "Cannot read properties of undefined
 * (reading 'client')" error. This check turns that into an actionable message.
 */
const assertCollectionShapeIsSupported = (collection: Collection): void => {
  const db = (collection as unknown as { db?: { client?: unknown } }).db;
  if (!db || !db.client) {
    throw new Error(
      "Your installed 'mongodb' driver version is too old for @langchain/mongodb.\n" +
        "Fix: run `npm install mongodb@^6.20.0` in the server folder, then delete " +
...
const getVectorStore = async () => {
  ...
  assertCollectionShapeIsSupported(collection);
